...
// Model for Personne entity
    models.Personne = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/ServiceForRestClient/webresources/model.personne/",
        idAttribute: 'id',
        defaults: {
            adresse: "",
            tel: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/ServiceForRestClient/webresources/model.personne/';
            }
    ...
    // Collection class for Personne entities
    models.PersonneCollection = Backbone.Collection.extend({
        model: models.Personne,
        url: "http://localhost:8080/ServiceForRestClient/webresources/model.personne/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.PersonneCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-personne-list-itemis template identifier for item
                        templateName: '#tpl-personne-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.personne = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.personne,
                    // tpl-personne-details is template identifier for chosen model element
                    templateName: '#tpl-personne-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Personne();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-personne-details is a template identifier for chosen model element
                templateName: '#tpl-personne-details',
